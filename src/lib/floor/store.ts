...
function log(
  ...
  return [
    { id: crypto.randomUUID(), at: now(), agent, kind, text, source, test },
    ...list,
  ].slice(0, 80);
}
...
export function healthOf(s: {
  ...
  if (s.mode === "open" && s.models.some((m) => m.loaded && m.role === "inbox")) {
    return { tone: "ok" as const, label: "All good", detail: "Jim is on the card." };
  }
...
export const useFloor = create((set: any, get: any) => ({
  ...
  kidMode: () => {
    set((s: any) => ({ mode: "kid", models: s.models.map((m: ModelSlot) => ({ ...m, loaded: false })) }));
    pushControl({ mode: "kid" });
  },
  ...
  runCreed: () => {
    set((s: any) => ({ mode: "creed", models: s.models.map((m: ModelSlot) => ({ ...m, loaded: m.role === "sunday" })) }));
    pushControl({ mode: "creed" });
  },
  ...
  ask: (raw: string) => {
    const reply = runFloorCommand(
      {
        pauseJim: get().pauseJim,
        warmJim: get().warmJim,
        openShop: get().openShop,
        closeShop: get().closeShop,
        kidMode: get().kidMode,
        nightOff: get().nightOff,
        runCreed: get().runCreed,
        stopCreed: get().stopCreed,
        startSystem: get().startSystem,
        stopSystem: get().stopSystem,
        setTestMode: get().setTestMode,
        jimTag: get().jimTag,
        flags: get().flags,
        queue: get().queue,
        models: get().models,
        scheduleEnabled: get().scheduleEnabled,
        shopOpenHour: get().shopOpenHour,
        shopCloseHour: get().shopCloseHour,
        health: () => healthOf(get()),
      },
      raw,
    );
